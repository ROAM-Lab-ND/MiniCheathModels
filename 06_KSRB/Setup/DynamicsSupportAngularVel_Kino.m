function SRBFuncs = DynamicsSupportAngularVel_Kino(SRB_param,mode)
...
w_des = SX.sym('ome', [3, 1]);         % Desired angular velocity in body frame
...
Qw  = SX.sym('Qome',[3, 1]);        % Weighting for Angular velocity
...
Qvec = [Qp;Qe;Qv;Qw;Qgs;Qgf;Qr];   % Vectorized Q matrix (diagnoal)
Rvec = [Rf;Rv];                     % Vectorized R matrix (diagnoal)


%       CoM Pos         Euler Angle             CoM Vel         Angular Velocity
l   = ws(p-p_des,Qp) + ws(eul-eul_des,Qe) + ws(v-v_des,Qv) + ws(w-w_des,Qw);
phi = ws(p-p_des,Qp) + ws(eul-eul_des,Qe) + ws(v-v_des,Qv) + ws(w-w_des,Qw);
